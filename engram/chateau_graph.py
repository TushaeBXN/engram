...
class ChateauGraph:
    ...
    Args:
        chateau: The active Chateau.
    """

    def __init__(self, chateau: Chateau) -> None:
    ...
    def build(self) -> None:
        ...
        self._adj.clear()
        for wing in self.chateau.list_wings():
            for tunnel in self.chateau.list_tunnels(wing.name):
                a = tunnel.room
    ...
    def suggest_links(self, min_rooms: int = 2) -> list[dict]:
        ...
        for wing in self.chateau.list_wings():
            for room in self.chateau.list_rooms(wing.name):
